...
namespace QuanLyPhong
{
    public partial class frmThongKe : Form
    {
        ...
        private string path_Excel = "C:\\Users\\UBC\\Desktop\\Các Môn Học\\4-Summer 2024\\Block - 2\\Dự Án 1\\Du_An_1_Nhom1\\File Thống Kê\\ThongKe.xlsx";
        ...
        private void check_dtGV_ThongKe(List<Orders> oders)
        {
            ...
            if (dtP_TuNgay.Value < dtP_DenNgay.Value && oders != null)
            {
                ...
                dtGV_ThongKe.Columns[8].Name = "Dịch Vụ Thêm";
                dtGV_ThongKe.Columns[9].Name = "Tổng Tiền";

                int Count = 0;

                //Hàng 1 --- Tổng Tiền
                var tongtienphong = oders.Sum(x => (x.ToTalPrice) - ((x.ToTalPrice * x.TotalDiscount) / 100));
                ...
                dtGV_ThongKe.Rows.Add("0", "All", "All", "All", "All", "All", tonggiaphong, tonggiaphong - tongtienphong, tongtiendichvu, tongtienphong + tongtiendichvu);
                //
                ...
                foreach (var item in oders)
                {
                    ...
                    dtGV_ThongKe.Rows.Add(Count, tenkh, item.PayMents, tennv, tenphong, item.DateCreated.Value.ToString("dd/MM/yyy"), item.ToTalPrice, item.ToTalPrice-item.ToTal, tienDichVu, item.ToTal + tienDichVu);
                }
            }
            else
            {
        ...
        private void Load_cbb_Phong()
        {
            ...
            foreach (var item in _roomService.GetAllRoomsFromDb())
            {
            ...
            if (cbb_Tang != null && cbb_Tang.SelectedIndex != 0)
            {
                ...
                var Phong = _roomService.GetAllRoomsFromDb().Where(x => x.FloorId == tangid).Select(x => x.RoomName);
                cbb_Phong.Items.Clear();
                cbb_Phong.Items.Add("Tất Cả");
                foreach (var item in Phong)
                {
                    cbb_Phong.Items.Add(item);
                }
            }
